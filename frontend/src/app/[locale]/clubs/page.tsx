...
export default function ClubsPage() {
  ...
  const applyToClubMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      alert(t('clubs.applicationSent'));
    },
    onError: (error: any) => {
      console.error('Erreur lors de l\'envoi de la candidature:', error);
      alert(t('clubs.applicationError') + ': ' + (error.response?.data?.message || error.message));
    }
  });
    ...
    <div className="min-h-screen bg-neutral-50 pb-20">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white py-12 px-4">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3">🏢 {t('clubs.title')}</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-neutral-600">{t('clubs.loading')}</p>
          </div>
                    ...
                    <div className="flex items-center gap-2 text-neutral-600 text-sm mb-2">
                      ...
                      {club.arenaCapacity && (
                        <span className="text-neutral-500">
                          ({club.arenaCapacity.toLocaleString()} {t('clubs.seats')})
                        </span>
                      )}
                    </div>
                  )}
                  ...
                  {club._count && club._count.events > 0 && (
                    <div className="mt-4 pt-4 border-t">
                      <div className="text-sm text-neutral-600">
                        📅 {club._count.events} {t('clubs.upcomingEvents', { count: club._count.events })}
                      </div>
                    </div>
                  )}
...
function ApplicationModal({ club, onClose, onSubmit, isLoading, t }: {
  club: Club;
  onClose: () => void;
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-neutral-800">
              📝 {t('clubs.applicationFor')} {club.name}
            </h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder={t('clubs.motivationPlaceholder')}
                required
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
              >
                <option value="">{t('clubs.selectPosition')}</option>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                rows={3}
                placeholder={t('clubs.experiencePlaceholder')}
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-2">
                {t('clubs.availability')}
              </label>
              <textarea
                ...
                className="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                rows={3}
                placeholder={t('clubs.availabilityPlaceholder')}
              />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {isLoading ? t('clubs.sending') : t('clubs.sendApplication')}
              </button>
            </div>
          </form>
        </div>
